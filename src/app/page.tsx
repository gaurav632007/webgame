...
function Character({ char, index }: { char: typeof characters[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.8, delay: char.delay, type: 'spring' as const, stiffness: 100, damping: 15 }}
      style={{ left: `${char.x}%`, top: `${char.y}%` }}
      className="absolute transform -translate-x-1/2 -translate-y-1/2"
    >
      <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center font-bold text-white text-lg sm:text-xl shadow-lg shadow-orange-500/25`}>
        {char.name.slice(0, 2)}
      </div>
      <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-medium text-gray-600 white-space-nowrap opacity-80">
        {char.name}
      </div>
    </motion.div>
  );
...
function FloatingParticle({ index }: { index: number }) {
  ...
  const colors = ['orange', 'pink', 'purple', 'blue', 'teal'];
...
const steps = [
  { number: 1, title: 'Create Room', description: 'Pick a mode, difficulty, and invite friends', icon: '🏠' },
  { number: 2, title: 'Get Your Role', description: 'Civilian knows the secret. Imposter does not.', icon: '🎭' },
  { number: 3, title: 'Give Clues', description: 'Describe the secret without being too obvious', icon: '💬' },
  { number: 4, title: 'Discuss', description: 'Chat, accuse, defend, and read the room', icon: '🗣️' },
  { number: 5, title: 'Vote', description: 'Choose who you think is the Imposter', icon: '🗳️' },
  { number: 6, title: 'Catch Them!', description: 'Reveal the Imposter and celebrate', icon: '🏆' },
];

const modes = [
  { icon: '🔍', name: 'Classic', desc: 'Standard Imposter game', color: 'from-orange-400 to-pink-500' },
  { icon: '🇮🇳', name: 'Desi Life', desc: 'Chai, cricket, Bollywood...', color: 'from-saffron-400 to-orange-500' },
  { icon: '🧠', name: 'Hardcore', desc: 'Similar, tricky topics', color: 'from-red-400 to-pink-500' },
  { icon: '🤪', name: 'Chaos', desc: 'Random rule twists', color: 'from-purple-400 to-pink-500' },
  { icon: '🖼️', name: 'Image Clue', desc: 'Picture-based clues', color: 'from-teal-400 to-cyan-500' },
  { icon: '👥', name: 'Custom', desc: 'Your own topic packs', color: 'from-blue-400 to-indigo-500' },
];
    ...
    <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          <section className="text-center mb-20 relative z-10">
            ...
            >
              ...
              <span className="block bg-gradient-to-r from-saffron-500 via-orange-500 to-pink-500 bg-clip-text text-transparent">
                Imposter
              </span>
            </motion.h1>
            ...
            >
              <Link href="/create">
                <Button size="xl" className="w-full sm:w-auto px-10 py-4 text-lg">
                  🎮 PLAY NOW
                </Button>
              </Link>
              <Link href="#how">
                <Button size="xl" variant="outline" className="w-full sm:w-auto px-10 py-4 text-lg">
                  📖 HOW TO PLAY
                </Button>
              </Link>
            </motion.div>
            ...
            >
              ...
              <span className="flex items-center gap-1">🎨 Fully Animated</span>
              <span className="flex items-center gap-1">🔊 Sound Effects</span>
              <span className="flex items-center gap-1">🇮🇳 Indian Themed</span>
            </motion.div>
          </section>
          
          <section id="how" className="mb-20 relative z-10">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
          ...
          <section id="modes" className="mb-20 relative z-10">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
                  ...
                  <Card className="h-full card-elevated hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-300 group">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${mode.color} opacity-0 group-hover:opacity-100 transition-opacity`} />
                      </div>
      ...
      <footer className="relative px-4 py-12 border-t border-orange-100">
        <div className="max-w-7xl mx-auto text-center text-gray-500 text-sm">
          <p className="mb-2">Made with 🇮🇳 for Indian parties everywhere</p>
          <p>Guess The Imposter © 2024</p>
        </div>
      </footer>
